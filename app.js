// =============================================
// MINERALES ESTRATÉGICOS — DATOS Y CONFIGURACIÓN
// =============================================
...
const MINERALS = [
  ...
  { key: 'Platino', unit: 'USD/kg', color: '#e2e8f0', gradient: ['rgba(226,232,240,.25)', 'rgba(226,232,240,0)'] },
...
// === DATOS EXACTOS DEL EXCEL: Precios_Minerales_50_Anos.xlsx ===
// Columnas: Año, Oro, Plata, Cobre, Hierro, Litio, Aluminio, Níquel, Platino, Paladio, Zinc
const rawData = [
...
MINERALS.forEach(m => {
  ...
  card.innerHTML = `
    ...
    <div class="kpi-value" style="color:${m.color}">${fmt(current)}</div>
    <div class="kpi-change ${isUp ? 'up' : 'down'}">${isUp ? '▲' : '▼'} ${Math.abs(change)}% vs 2025</div>
  `;
  kpiStrip.appendChild(card);
});

// =============================================
// GRÁFICA GENERAL — TODOS LOS MINERALES
// =============================================
const generalCtx = document.getElementById('generalChart').getContext('2d');
const generalDatasets = MINERALS.map(m => ({
  label: m.key,
  data: mineralData[m.key],
  borderColor: m.color,
...
let generalChart = new Chart(generalCtx, {
  type: 'line',
  data: { labels: years, datasets: generalDatasets },
  options: {
    responsive: true,
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: ctx => `${ctx.dataset.label}: ${fmt(ctx.parsed.y)} USD/kg`
        }
      }
    },
    scales: {
      x: { 
        ...
        ticks: { 
          ...
          callback: function(value, index) {
            ...
            if (year === 1976 || year === 2026 || year % 5 === 0) return year;
            return '';
          }
        }, 
        border: { display: false } 
      },
      y: { type: 'logarithmic', grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: '#64748b', font: { size: 11 }, callback: v => fmt(v) }, border: { display: false } }
    },
...
MINERALS.forEach((m, i) => {
  ...
  generalLegend.appendChild(item);
});

// =============================================
// GRÁFICAS INDIVIDUALES POR MINERAL
// =============================================
...
MINERALS.forEach((m, idx) => {
  ...
  card.innerHTML = `
    <div class="chart-header">
      <div>
        <div class="mineral-name"><span class="dot" style="background:${m.color}"></span>${m.key}</div>
        <div class="mineral-unit">${m.unit} · 1976–2026</div>
      </div>
      <div class="mineral-stats">
        ...
        <div class="stat"><div class="stat-label">2026</div><div class="stat-val" style="color:#22c55e">${fmt(current)}</div></div>
      </div>
    </div>
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            title: ctx => `Año ${ctx[0].label}`,
            label: ctx => `${m.key}: ${fmt(ctx.parsed.y)} ${m.unit}`
          }
        }
      },
      scales: {
        x: { 
          ...
          ticks: { 
            ...
            callback: function(value, index) {
              const year = years[index];
              if (year === 1976 || year === 2026 || year % 10 === 0) return year;
              return '';
            }
          }, 
          border: { display: false } 
        },
        y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: '#64748b', font: { size: 10 }, callback: v => fmt(v) }, border: { display: false } }
      },
      interaction: { intersect: false, mode: 'index' }
    }
  });
...
// =============================================
// TABLA DE PRODUCTORES (USGS MCS 2025 + Fuentes 2024-2025)
// =============================================
const producersData = [
...
producersData.forEach((row, i) => {
  ...
  const mc = MINERALS[i].color;
  tr.innerHTML = `<td class="rank-cell">${i + 1}</td><td class="mineral-cell" style="color:${mc}">${row.mineral}</td>` +
